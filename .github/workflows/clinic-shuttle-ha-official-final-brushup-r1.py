...
EVID=ROOT/'release-evidence-ha-official'
...
CSS_MARKER='/* CLINIC SHUTTLE HUMAN ACCEPTANCE DENSITY R1 */'
CSS_BLOCK='''
/* CLINIC SHUTTLE HUMAN ACCEPTANCE DENSITY R1 */
.clinic-shuttle-product-page .section{padding-top:clamp(48px,5.4vw,68px);padding-bottom:clamp(48px,5.4vw,68px)}
.clinic-shuttle-product-page #docs{padding-bottom:38px}
.clinic-shuttle-product-page #faq{padding-top:38px}
.clinic-shuttle-product-page .product-live#demo{padding-top:clamp(44px,5vw,62px);padding-bottom:clamp(48px,5vw,66px)}
.clinic-shuttle-product-page .product-section-heading{margin-bottom:clamp(22px,2.8vw,32px)}
.clinic-shuttle-product-page .clinic-docs a{min-height:70px;display:flex;align-items:center}
@media(max-width:560px){
  .clinic-shuttle-product-page .section{padding-top:42px;padding-bottom:42px}
...
def fetch(url,timeout=45):
    req=urllib.request.Request(url,headers={'User-Agent':'Mozilla/5.0 DPRO-HA-OFFICIAL'})
...
def patch():
    ...
    for old,new,label in [
        (REMOVE_STAGE,'','hero duplicate live-demo copy'),
        (OLD_DEMO,NEW_DEMO,'demo heading'),
        (OLD_DOC,NEW_DOC,'docs heading'),
        (REMOVE_FAQ,'','duplicate FAQ'),
        (OLD_FINAL,NEW_FINAL,'final CTA copy')
    ]:
        c=s.count(old)
        if c!=1: raise SystemExit(f'{label} expected exactly once, got {c}')
    ...
    if not checks['duplicate_phrase_removed'] or not checks['hero_tablet_removed'] or not checks['demo_heading'] or checks['faq_count']!=3 or not checks['official_price'] or not checks['product_link'] or not checks['demo_link']:
        raise SystemExit('source checks failed '+json.dumps(checks,ensure_ascii=False))
    write('HA_OFFICIAL_SOURCE_R1.json',{
      'pass':True,
      'humanAcceptanceDefects':['HA-01','HA-02','HA-03','HA-06'],
      'changes':['Removed duplicate hero LIVE DEMO explanatory block','Changed live section heading to role-specific current-screen purpose','Reduced section/docs/FAQ vertical spacing','Removed duplicate live-demo FAQ','Clarified final CTA role separation'],
      'checks':checks,'systemReopenRequired':False
    })
    print('HA OFFICIAL SOURCE PASS')
...
def publicqa():
    ...
    if not re.fullmatch(r'[0-9a-f]{40}',head): raise SystemExit('FINAL_OFFICIAL_HEAD missing')
    ...
    with sync_playwright() as pw:
        ...
        for w in [375,390,768,1440]:
            ...
            p.goto(URL,wait_until='networkidle',timeout=60000); p.wait_for_timeout(350)
            p.evaluate('''async()=>{for(let y=0;y<document.body.scrollHeight;y+=700){window.scrollTo(0,y);await new Promise(r=>setTimeout(r,25));}window.scrollTo(0,0)}''')
            rec=p.evaluate('''()=>({scrollWidth:document.documentElement.scrollWidth,innerWidth:window.innerWidth,height:document.documentElement.scrollHeight,broken:[...document.images].filter(i=>!i.complete||i.naturalWidth===0).map(i=>i.src),hiddenReveal:[...document.querySelectorAll('.reveal')].filter(x=>{const c=getComputedStyle(x),r=x.getBoundingClientRect();return c.display==='none'||c.visibility==='hidden'||Number(c.opacity)<.95||r.width<=0||r.height<=0}).length,details:document.querySelectorAll('#faq details').length,duplicateText:document.body.innerText.includes('説明画像ではなく'),heroTablet:document.querySelectorAll('.product-stage-tablet').length})''')
            ...
            if w in (390,1440): p.screenshot(path=str(EVID/f'official-ha-r1-{w}.png'),full_page=True)
            qa[str(w)]=rec; p.close()
        b.close()
    if api_head('dpromstk2000-lab/dpro-clinic-shuttle-line')!=SYSTEM_HEAD: raise SystemExit('SYSTEM FINAL LOCK drift')
    if api_head('dpromstk2000-lab/dpro-line-systems-site')!=PRODUCT_HEAD: raise SystemExit('PRODUCT drift during OFFICIAL HA fix')
    write('HA_OFFICIAL_PUBLIC_QA_R1.json',{'pass':True,'finalOfficialHead':head,'pages':pages,'viewports':qa,'systemFinalLock':SYSTEM_HEAD,'productHead':PRODUCT_HEAD,'humanAcceptanceStatus':'PENDING_PRODUCT_FIX'})
    print('HA_OFFICIAL_R1_SUMMARY='+json.dumps({'status':'PASS','final_official_head':head,'pages_run':pages['id'],'system_final_lock':SYSTEM_HEAD,'human_acceptance':'PENDING_PRODUCT_FIX'},ensure_ascii=False))

if __name__=='__main__':
    if len(sys.argv)!=2: raise SystemExit('usage: patch|publicqa')
