(() => {
  ...
  const inspectCatalog = async () => {
    try {
      ...
      const positions = Array.isArray(itemList?.itemListElement)
        ...
        : [];
      const sequential = positions.length === 44 && positions.every((value, index) => value === index + 1);
      const ok = response.ok && cards.length === 44 && unique.size === 44 && !missing.length &&
        !extra.length && filters.length === 12 && itemList?.numberOfItems === 44 && sequential;
      addResult(
        "一覧統合",
        "44カード・11カテゴリ・ItemList",
        ok ? "pass" : "fail",
      ...
      addResult("一覧統合", "44カード・11カテゴリ・ItemList", "fail", error.message);
    }
  };
  ...
  const inspectSitemap = async () => {
    try {
      ...
      const ok = response.ok && !parserError && locs.length === 54 && unique.size === 54 &&
      ...
      addResult("検索エンジン", "sitemap.xml 54URL", "fail", error.message);
    }
  };
  ...
  const runChecks = async () => {
    ...
    updateCounts();
    setProgress("検査を開始します", `主要10ページ＋システム44ページ`);
