(() => {
  ...
  const SYSTEM_SLUGS = ["vet", "pet-salon", "dental", "osteopathic", "dayservice", "home-nursing", "homecare", "houkago-dayservice", "btype", "sodan", "welfare-equipment", "shuttle", "caretaxi", "hair", "salon", "nail", "eye-salon", "esthe", "gym", "yoga", "bakery", "cake", "takeout", "izakaya", "yakiniku", "buyback", "cosmetics", "flower-shop", "photo-studio", "repair", "car-service", "used-car", "estate", "reform", "disposal", "school", "stay", "cleaning", "housekeep", "gyosei", "shiho", "sharoushi", "tax-accounting", "chosashi", "funeral"];
  const SYSTEM_NAMES = {"vet": "動物病院", "pet-salon": "ペットサロン", "dental": "歯科医院", "osteopathic": "整骨院・整体", "dayservice": "デイサービス", "home-nursing": "訪問看護", "homecare": "訪問介護・家族連絡", "houkago-dayservice": "放課後等デイ", "btype": "就労継続支援B型", "sodan": "相談支援事業所", "welfare-equipment": "福祉用具", "shuttle": "福祉施設送迎", "caretaxi": "介護タクシー", "hair": "美容室", "salon": "美容サロン", "nail": "ネイルサロン", "eye-salon": "まつげ・眉サロン", "esthe": "エステ・リラク", "gym": "パーソナルジム", "yoga": "プライベートヨガ", "bakery": "ベーカリー", "cake": "ケーキ・洋菓子", "takeout": "テイクアウト", "izakaya": "居酒屋", "yakiniku": "焼肉店", "buyback": "買取・査定", "cosmetics": "化粧品店", "flower-shop": "フラワーショップ", "photo-studio": "写真館", "repair": "修理受付", "car-service": "車検・整備工場", "used-car": "中古車買取・販売", "estate": "不動産・賃貸内見", "reform": "リフォーム・工務店", "disposal": "不用品回収・遺品整理", "school": "学習塾・習い事", "stay": "宿泊・民泊", "cleaning": "クリーニング", "housekeep": "家事代行", "gyosei": "行政書士", "shiho": "司法書士・相続登記", "sharoushi": "社労士", "tax-accounting": "税理士・会計", "chosashi": "土地家屋調査士", "funeral": "葬儀・法要"};
  ...
  const inspectCatalog = async () => {
    try {
      ...
      const positions = Array.isArray(itemList?.itemListElement)
        ...
        : [];
      const sequential = positions.length === 45 && positions.every((value, index) => value === index + 1);
      const ok = response.ok && cards.length === 45 && unique.size === 45 && !missing.length &&
        !extra.length && filters.length === 12 && itemList?.numberOfItems === 45 && sequential;
      addResult(
        "一覧統合",
        "45カード・11カテゴリ・ItemList",
        ok ? "pass" : "fail",
      ...
      addResult("一覧統合", "45カード・11カテゴリ・ItemList", "fail", error.message);
    }
  };
  ...
  const inspectSitemap = async () => {
    try {
      ...
      const ok = response.ok && !parserError && locs.length === 55 && unique.size === 55 &&
      ...
      addResult("検索エンジン", "sitemap.xml 55URL", "fail", error.message);
    }
  };
  ...
  const runChecks = async () => {
    ...
    updateCounts();
    setProgress("検査を開始します", `主要10ページ＋システム45ページ`);
